...
internal sealed class AppointmentRepository : IAppointmentRepository
{
    ...
    public Task<PagedResult<AppointmentWithDetailsDto>> GetForPatientWithDetailsAsync(
        Guid patientUserId,
        int page,
        int pageSize,
        CancellationToken cancellationToken)
        => GetWithDetailsAsync(a => a.PatientUserId == patientUserId, page, pageSize, cancellationToken);

    public Task<PagedResult<AppointmentWithDetailsDto>> GetForDoctorWithDetailsAsync(
        Guid doctorUserId,
        int page,
        int pageSize,
        CancellationToken cancellationToken)
        => GetWithDetailsAsync(a => a.DoctorUserId == doctorUserId, page, pageSize, cancellationToken);

    private async Task<PagedResult<AppointmentWithDetailsDto>> GetWithDetailsAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var baseQuery = _db.Appointments.AsNoTracking().Where(predicate);
