...
import { useQuery } from "@tanstack/react-query";
import axiosClient from "@/lib/axios";
import { doctorsApi } from "../api/doctors-api"; // Imported to fetch doctor by ID
import { 
  useDoctorRatings, 
  useUpdateRating, 
  useDeleteRating 
...
export function DoctorProfileView({ doctor: initialDoctor, onBack, onProceedToBooking }: DoctorProfileViewProps) {
  // Triggers the successful GET /api/doctors/{id} request in the network tab
  const { data: doctorData } = useQuery({
    queryKey: ["doctor-profile", initialDoctor.userId],
    queryFn: () => doctorsApi.getDoctorById(initialDoctor.userId),
    initialData: initialDoctor, // Instantly displays list data while fetching in the background
  });

  const doctor = doctorData || initialDoctor;
  const fullName = `Dr. ${doctor.firstName} ${doctor.lastName}`;
...
function PatientNameLookup({ userId }: { userId: string }) {
  // Bypassed the /api/users/{id} API call to prevent 404 network errors.
  return (
    <span className="font-bold text-slate-800 bg-slate-100 px-2 py-0.5 rounded-md">
      Verified Patient
    </span>
