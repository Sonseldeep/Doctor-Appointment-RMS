...
// import { useExportPrescription } from "@/features/clinical-notes/hooks/use-export-prescription";

// export default function PrescriptionHistoryPage() {
//   const { data: history, isLoading: isHistoryLoading } = useGetClinicalNotesHistory();
...
//   const { exportPdf, isLoading: isExporting } = useExportPrescription();
...
//                             className={`p-4 rounded-xl border transition-all text-left flex justify-between items-center group w-full ${
//                               isHighlighted
//                                 ? "bg-blue-50/60 ring-1 ring-inset ring-blue-100 border-l-4 border-l-blue-500 border-y-blue-200 border-r-blue-200 shadow-sm"
//                                 : "bg-white border-slate-200 hover:border-blue-300 hover:shadow-md active:scale-[0.99]"
//                             }`}
//                           >
//                             <div className="flex items-center gap-3">
//                               <div className="w-9 h-9 flex items-center justify-center rounded-lg bg-slate-100 group-hover:bg-blue-50 text-slate-500 group-hover:text-blue-600 transition-colors overflow-hidden shrink-0">
//                                  {(isDoctor ? note.patientPhotoUrl : note.doctorPhotoUrl) ? (
//                                    <img 
//                                      src={isDoctor ? note.patientPhotoUrl : note.doctorPhotoUrl} 
//                                      alt="Profile" 
//                                      className="w-full h-full object-cover"
//                                    />
//                                  ) : (
...
//                                 </h4>
//                                 <p className="text-[10px] text-blue-600 font-bold bg-blue-50/70 inline-block px-1.5 py-0.5 rounded border border-blue-100/50">
//                                   {note.diagnosis}
//                                 </p>
//                               </div>
//                             </div>
...
export default function PrescriptionHistoryPage() {
  ...
  useEffect(() => {
    if (history && targetNoteId && !selectedNote) {
      ...
      if (matchingNote) {
        ...
        // Auto-expand the correct doctor/patient group if they return to list view
        const personKey = isDoctor ? (matchingNote.patientId || matchingNote.patientName) : (matchingNote.doctorId || matchingNote.doctorName);
        setExpandedId(personKey);
      }
    }
  }, [history, targetNoteId, selectedNote, isDoctor]);
  ...
  // 1. Filter logic remains unchanged
  const filteredHistory = history?.filter((note: any) => {
    if (!dateFilter) return true;
    const rawDate = note.createdAt || note.date || note.followUpDate;
    if (!rawDate) return false;
  ...
  // 2. NEW LOGIC: Group records by Doctor (or Patient if user is Doctor)
  const groupedByPerson = filteredHistory?.reduce((acc: any, note: any) => {
    const personKey = isDoctor ? (note.patientId || note.patientName) : (note.doctorId || note.doctorName);
    const personName = isDoctor ? note.patientName : note.doctorName;
    const personPhotoUrl = isDoctor ? note.patientPhotoUrl : note.doctorPhotoUrl;
    ...
    acc[personKey].notes.push(note);
    return acc;
  }, {} as Record<string, { id: string, name: string, photoUrl: string, notes: any[] }>) || {};

  // Sort notes chronologically within each group (newest first)
  Object.values(groupedByPerson).forEach((group: any) => {
    group.notes.sort((a: any, b: any) => {
      const dateA = new Date(a.createdAt || a.date || a.followUpDate || 0).getTime();
      const dateB = new Date(b.createdAt || b.date || b.followUpDate || 0).getTime();
      return dateB - dateA;
    });
  });
      ...
      <div>
        {isHistoryLoading ? (
          ...
          <div className="space-y-6 w-[95%] mx-auto">
            ...
            {/* Refactored Master View (Grouped) */}
            <div className="space-y-4">
              {Object.keys(groupedByPerson).length === 0 ? (
                ...
                Object.values(groupedByPerson).map((group: any) => {
                  const isExpanded = expandedId === group.id;

                  // Group the internal timeline by Date purely for a cleaner nested display
                  const groupedNotesByDate = group.notes.reduce((acc: any, note: any) => {
                    const rawDate = note.createdAt || note.date || note.followUpDate;
                    const formattedDate = rawDate 
                      ? new Date(rawDate).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })
                      : "General History";
                  ...
                  return (
                    <div 
                      key={group.id} 
                      className={`bg-white border rounded-2xl overflow-hidden transition-all duration-300 ${isExpanded ? 'border-blue-200 shadow-md ring-1 ring-blue-50' : 'border-slate-200 shadow-sm hover:border-slate-300 hover:shadow'}`}
                    >
                      {/* Interactive Master Card */}
                      <button 
                        onClick={() => setExpandedId(isExpanded ? null : group.id)}
                        className="w-full p-4 md:p-5 flex items-center justify-between bg-white hover:bg-slate-50/50 transition-colors focus:outline-none"
                      >
                        <div className="flex items-center gap-4">
                          <div className={`w-12 h-12 flex items-center justify-center rounded-xl overflow-hidden shrink-0 transition-colors ${isExpanded ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-500'}`}>
                            {group.photoUrl ? (
                          ...
                          <div className="text-left">
                            <div className="flex items-center gap-2">
                              ...
                              {isDoctor && <Stethoscope size={14} className="text-slate-400" />}
                            </div>
                            <p className="text-[11px] font-bold text-slate-500 mt-1 uppercase tracking-wider">
                              <span className="text-blue-600 bg-blue-50/70 px-2 py-0.5 rounded border border-blue-100/50 mr-1.5">
                                {group.notes.length} {group.notes.length === 1 ? 'Record' : 'Records'}
                              </span>
                            </p>
                          </div>
                        </div>
                        <div className={`p-2 rounded-full transition-all duration-300 ${isExpanded ? 'rotate-90 bg-blue-50 text-blue-500' : 'bg-transparent text-slate-400'}`}>
                          <ChevronRight size={20} />
                        </div>
                      </button>

                      {/* Detail View (Nested Timeline) utilizing CSS Grid Animation */}
                      <div className={`grid transition-all duration-300 ease-in-out ${isExpanded ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                        <div className="overflow-hidden">
                          <div className="p-5 md:p-6 pt-2 bg-slate-50/50 border-t border-slate-100">
                            
                            <div className="space-y-6 relative pl-5 border-l-2 border-slate-200 ml-3 mt-4">
                              {Object.entries(groupedNotesByDate).map(([dateGroup, items]: [string, any]) => (
                                <div key={dateGroup} className="space-y-3 relative mb-6 last:mb-0">
                                  <div className="absolute -left-[28.5px] top-1 bg-white w-3.5 h-3.5 rounded-full border-2 border-blue-500 ring-4 ring-slate-50" />
                                  
                                  <div className="flex items-center gap-1.5 mb-1 py-0.5 w-max">
                                    <Calendar size={13} className="text-slate-400" />
                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{dateGroup}</span>
                                  </div>
                                  ...
                                  <div className="grid gap-2.5">
                                    {items.map((note: any) => {
                                      ...
                                      return (
                                        ...
                                        >
                                          {/* Contextual Child Card - Removed repetitive Avatar/Name */}
                                          <div className="space-y-1">
                                            ...
                                            <div className="flex items-center gap-2 text-[11px] text-slate-500">
                                              <span className="truncate max-w-[180px] sm:max-w-[250px] font-medium">
                                                {note.treatmentSummary || "No detailed treatment summary provided"}
                                              </span>
                                              {note.medications?.length > 0 && (
                                                <>
                                                  <span className="w-1 h-1 rounded-full bg-slate-300 shrink-0"></span>
                                                  <span className="text-blue-600 font-bold shrink-0">{note.medications.length} Meds</span>
                                                </>
                                              )}
                                            </div>
                                          </div>
                                          <ChevronRight size={16} className="text-slate-400 group-hover:text-blue-500 group-hover:translate-x-0.5 transition-all shrink-0 ml-3" />
                                        </button>
                                      );
                                    })}
                                  </div>
